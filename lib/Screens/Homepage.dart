...
class Dashboard {
  ...
  Dashboard(this.image, this.type, this.value, this.valuename, this.percentage,
      this.note, this.raitngcount, this.percvalue);
}
...
Future<void> refreshData() async {
      ...
      APIManager().apiRequest(context, API.attendance, (response) async {
        ...
        if (resp.status == 1) {
          setState(() {
            ...
            log('GlobalLists.attendancedata.count ${GlobalLists.attendancedata}');
  ...
  getworkeflow() async {
    ...
    if (role == GlobalLists.operationrole ||
      ...
      log('called');
      //supervisor
      // await workflowstatusApi();
      // background.workflowstatusApi(GlobalLists.siteid, context);
  ...
  operationlworkflowstatusApi() async {
    ...
    APIManager().apiRequest(context, API.operationalworkflow, (response) async {
    ...
    return WillPopScope(
      onWillPop: () async {
        ShowDialogs.showConfirmDialog(
            context, "Exit", "Are you sure you want to\n quit the application?",
            () {
          SystemNavigator.pop();
        });
        return true;
                                ...
                                setStateDialgoue(() {
                                  ...
                                  for (int i = 0; i < agendalist.length; i++) {
                                    if (agendalist[i].isselected) {
                                      agelist
                                          .add(agendalist[i].name.toString());
                      ...
                      GestureDetector(
                        onTap: () {
                          setStateDialgoue(() {
                            ...
                            attendancesiteid =
                                GlobalLists.sitemasterlist[index].id.toString();
                            isexpanded = false;
                          });
                        },
                        child: Container(
                      ...
                      GestureDetector(
                        onTap: () {
                          setStateDialgoue(() {
                            ...
                            namecontroller.text = "";
                            janotoragendaApi(
                                attendanceclientid, attendancesiteid);
                          });
                        },
                        child: Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    GlobalLists
                                        .unitlowattendacelist[index].clientName,
                                    maxLines: 2,
                                    textAlign: TextAlign.start,
                                    overflow: TextOverflow.ellipsis,
                                    style: AppFonts.headerwithletterStyle(
                                                                        ...
                                                                        children:
                                                                          ...
                                                                          (int
                                                                            ...
                                                                            return GestureDetector(
                                                                              onTap: () async {
                                                                                print("openfile");
                                                                                showfileimage(result[index].split('/').last, result[index]);
                                                                                // Navigator.push(
                                                                                //     context,
                                                                                //     MaterialPageRoute(
                                                                              ...
                                                                              child: Chip(
                                                                                side: BorderSide(style: BorderStyle.solid, color: customcolor.blue),
...
//  uploadcontroller.text=_fileName!;
  ...
  Widget headcard() {
    ...
    : CircularPercentIndicator(
        ...
        radius: 28.0, // matches loading circle for smooth transition
        lineWidth: 4.0,
        animation: true,
        percent: (headlist[index].percvalue ?? 0.0).clamp(0.0, 1.0),
        circularStrokeCap: CircularStrokeCap.round,
        progressColor: customcolor.textblue,
    ...
    //     children: [
    //       SizedBox(
    //         height: 60, // match your actual indicator size
    //         width: 60,
    //         child: CircularProgressIndicator(
    //           strokeWidth: 5,
    //           valueColor: AlwaysStoppedAnimation(customcolor.textblue),
    ...
    //                               isFirstLoad
    // // WHEN FIRST LOAD -> show loader inside circle (static text already visible)
    // ? Row(
    //     children: [
    //       SizedBox(width: 10),
    //       SizedBox(
    ...
    //     ],
    //   )
    // // AFTER FIRST LOAD -> show actual percent circular indicator (with percent value)
    // : CircularPercentIndicator(
    //     animationDuration: 500,
    //     lineWidth: 5.0,
    //     radius: 30.0,
    ...
    //   ),
                                  ...
                                  //         //   lineWidth: 5.0,
                                  //         animation: true,
                                  //         percent: headlist[index].percvalue,
                                  //         center: Text(
    ...
    ? Stack(
        ...
        children: [
          ...
          CircularPercentIndicator(
            lineWidth: 4.0,
            radius: 28.0, // slightly smaller for better alignment
            percent: 1.0,
            backgroundColor: Colors.transparent,
            progressColor: Colors.grey.shade300,
            circularStrokeCap: CircularStrokeCap.round,
          ...
          SizedBox(
            height: 52, // slightly smaller than before (was 60)
            width: 52,
            child: CircularProgressIndicator(
              strokeWidth: 4.0,
              valueColor: AlwaysStoppedAnimation(customcolor.textblue),
    ...
    : CircularPercentIndicator(
        animationDuration: 500,
        radius: 28.0, // match loading radius for seamless visual transition
        lineWidth: 4.0,
        animation: true,
        percent: role == GlobalLists.clientrole
            ? (GlobalLists.unitdashboard.visitCount ?? 0) > 0
                ? 1.0
                : 0.0
            : ((GlobalLists.unitdashboard.visitCount ?? 0) /
                    (GlobalLists.unitdashboard.totalSiteCount == 0
                        ? 1 // avoid divide-by-zero
                        : GlobalLists.unitdashboard.totalSiteCount))
                .clamp(0.0, 1.0),
        center: Text(
          role == GlobalLists.clientrole
    ...
    APIManager().apiRequest(context, API.ongoingshift, (response) async {
      try {
        ...
        if (resp.status == 1) {
          ...
          await prefs.setString(cacheKey, dashboardlistResponseToJson(resp));
...
//   }
  ...
  void updateDashboardState(DashboardlistResponse resp) {
    ...
    GlobalLists.totalattendanceper = double.parse(GlobalLists.noofstaff) *
...
Future<void> unitdashboardApi() async {
  ...
  if (cachedData != null) {
    try {
      UnitDashboardResponse cachedResp =
          UnitDashboardResponse.fromJson(jsonDecode(cachedData));
      setState(() {
  ...
  if (headlist.isEmpty) {
    setState(() {
      headlist = buildDashboardListPlaceholder(); // ensures text/icons visible
  ...
  if (isConnected) {
    APIManager().apiRequest(context, API.unitdashboard, (response) async {
      try {
  ...
  List<Dashboard> buildDashboardListPlaceholder() {
    ...
    bool isOperation = role == GlobalLists.operationrole ||
        role == GlobalLists.operationmanagerrole;
    bool isHead =
        role == GlobalLists.headrole || role == GlobalLists.reginalmanagerrole;

    if (isOperation || isHead || isClient) {
      list.add(Dashboard("assets/images/image4.png", "ATTENDANCE", "",
          "Sites with full attendance", "--", "", "", 0.0));
      list.add(Dashboard("assets/images/image3.png", "WORKFLOW", "",
          "Percentage of workflow \ncompleted", "--", "", "", 0.0));
      list.add(Dashboard("assets/images/image1.png", "TRAINING", "",
          "Sites with training \ncompleted", "--", "", "", 0.0));
      list.add(Dashboard("assets/images/image2.png", "RATING", "",
          "Average Rating given by clients", "--", "", "", 0.0));
    }
  ...
  List<Dashboard> buildDashboardList(UnitDashboardResponse resp) {
    ...
    if (isOperation) {
      ...
      list.add(Dashboard(
          ...
          (double.parse(resp.ratingCount.toStringAsFixed(0)) / 10 * 100) /
              100));
    } else if (isHead) {
  ...
  clientdashboardApi() async {
    ...
    if (status1) {
      ...
      if (cachedData != null) {
        clientdash.ClientsiteDashboardResponse cachedResp =
            clientdash.clientsiteDashboardResponseFromJson(cachedData);
        setState(() {
  ...
  unitclientmasterApi() async {
    ...
    if (status1) {
      ...
      if (savedData != null) {
        try {
          UnitclientMasterResponse resp =
              UnitclientMasterResponse.fromJson(jsonDecode(savedData));
          if (resp.status == 1) {
            setState(() {
              GlobalLists.clientmasterlist = resp.data;
            });
  ...
  unitsitemasterApi(String clientid) async {
    ...
    if (status1) {
      ...
      APIManager().apiRequest(context, API.unitsitemaster, (response) async {
...
setState(() {
  addTrainingLoad=true;
      ...
      var request = http.MultipartRequest(
          "POST", Uri.parse(APIManager.operationaladdtraining));
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      ...
                      GestureDetector(
                        ...
                        child: Align(
                          alignment: Alignment.bottomRight,
                          child:addAttendance? CircularProgressIndicator(color: customcolor.blue,): Image.asset(
...
setState(() {
  addAttendance=true;
      ...
      APIManager().apiRequest(context, API.addattendance, (response) async {
  ...
  addAttendance=false;
            ...
            Timer(Duration(seconds: 1), () => Navigator.pop(context));
            ShowDialogs()
                .confirmationdone(context, "Mark Attendance \nSuccessfully");
            Timer(
                Duration(seconds: 1),
                () => Navigator.push(
                      context,
                              ...
                              onChanged: (value) {
                                setStateDialgoue(() {
                                  ...
                                  for (int i = 0;
                                    ...
                                    if (dropdownList[i].isselected) {
                                      agelist
                                          .add(dropdownList[i].name.toString());
                                      agendais
                                          .add(dropdownList[i].id.toString());
                                    }
                                  }
                                  String s = agelist.join(', ');
